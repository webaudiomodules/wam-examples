...
`webaudioplugins` field is a map of [packageName, buildDirectory].  
...
- `index.js`: an ES module that implements the _WebAudioPlugin_ class from the sdk
...
export default class SimpleGainPlugin extends WebAudioPlugin {
...
export async function createElement(plugin) {
	...
	div.textContent = `WebAudioPlugin[${plugin.name}]`;
	return div;
}
```
...
```js
{
	...
	// the relative path to WebAudioPlugin module (default: index.js)
	"entry": "./index.js",
	// the relative path to WebAudioPlugin module (default: gui.js)
	"gui": "./Gui/index.js"
}
```
