	...
	async createAudioNode(initialState) {
		...
		const internalParamsConfig = {
			enabled: {
				...
				minValue: 0,
				maxValue: 1,
				onChange: (value) => { graphicEQNode.status = !!value; },
			},
		};
		/**
		...
		graphicEQNode.filters.forEach((filter, index) => {
			const {
				type, Q, detune, frequency, gain,
			} = filter;
			internalParamsConfig[`${type}_${index}_Q`] = { ...getParamConfigFromAudioParam(Q), onChange: (value) => { Q.value = value; } };
			internalParamsConfig[`${type}_${index}_detune`] = { ...getParamConfigFromAudioParam(detune), onChange: (value) => { detune.value = value; } };
			internalParamsConfig[`${type}_${index}_frequency`] = { ...getParamConfigFromAudioParam(frequency), onChange: (value) => { frequency.value = value; } };
			internalParamsConfig[`${type}_${index}_gain`] = { ...getParamConfigFromAudioParam(gain), onChange: (value) => { gain.value = value; } };
		});
