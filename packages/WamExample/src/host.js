...
(async () => {
	...
	// You can can optionally give more options such as the initial state of the plugin
	...
	player.onplay = () => {
		...
		setTimeout(() => {
			console.log('After 2.5s, set gain to 0.0...');
			// set gain after 5 seconds
			window.instance.audioNode.setParameterValues({ gain: { id: 'gain', value: 0.0, interpolate: true } });
			// store current state
			state = window.instance.audioNode.getState();
