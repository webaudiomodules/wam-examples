...
<p>MidiOut : <select id="midiout"><option>WebAudioTinySynth</option></select></p>
...
export default class MidiVirtualKeyboardHTMLElement extends HTMLElement {
	...
	async setKnobs() {
		...
		this.synth = this.shadowRoot.querySelector('#synth');

		await this.synth.ready();
		this.displayTimbreName(0);
	}
	...
	displayTimbreName(val) {
		const name = this.synth.getTimbreName(0, val);
		this.shadowRoot.querySelector('#timbrename').innerText = name;
		let prog = this.shadowRoot.querySelector('#prog');
		prog.setAttribute
	}

	sendMidiMessage(mess) {
		if (this.midiout) this.midiout.send(mess);
		else this.synth.send(mess);
	}
	...
	static is() {
		return 'wasabi-midivirtualkeyboard-without-builder';
	}
}

if (!customElements.get(MidiVirtualKeyboardHTMLElement.is())) {
	customElements.define(
		MidiVirtualKeyboardHTMLElement.is(),
		MidiVirtualKeyboardHTMLElement
	);
}
