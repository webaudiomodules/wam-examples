...
/**
 * @typedef {import('../api/src').WebAudioModule} WebAudioModule
...
Array.from(examples).forEach((example) => {
	example.addEventListener('click', () => {
		const pluginUrl = `../${example.dataset.pluginUrl}/index.js`;
...
Array.from(midiEmitters).forEach((midiEmitter) => {
	midiEmitter.addEventListener('click', () => {
		const pluginUrl = `../${midiEmitter.dataset.pluginUrl}/index.js`;
